...
void ViewerWidget::exportThreeViewDrawing()
{
    if (!m_occView)
        return;
    ...
    Handle(Graphic3d_Camera) savedCamera = new Graphic3d_Camera();
    savedCamera->Copy(currentCamera);

    // Ensure OpenGL context is active
    m_occView->makeCurrent();

#if SetupLight
    // Setup Export Lighting to avoid "all white" saturation
    // Turn off existing lights
    view->SetLightOff();

    // Create temporary export lights
    Handle(V3d_AmbientLight) ambLight = new V3d_AmbientLight(Quantity_NOC_WHITE);
    ambLight->SetIntensity(0.5); // Moderate ambient
    view->SetLightOn(ambLight);

    Handle(V3d_DirectionalLight) headLight = new V3d_DirectionalLight(
        gp_Dir(0.0, 0.0, -1.0), Quantity_NOC_WHITE, Standard_True); // Headlight
    headLight->SetIntensity(0.7);                                   // Moderate directional
    view->SetLightOn(headLight);
#endif

    // Use current directory for saving
    QString baseName = "threeview";

    // Helper to snap and save using ToPixMap which is more reliable for offscreen content
    auto captureView = [&](V3d_TypeOfOrientation orientation, const QString &suffix) {
        view->SetProj(orientation);
        view->FitAll(0.01, false); // Fit with small margin

        Image_AlienPixMap pixmap;
        // Use exact widget dimensions
        if (view->ToPixMap(pixmap, m_occView->width(), m_occView->height())) {
            pixmap.Save((baseName + suffix + ".png").toStdString().c_str());
        }
    };

    captureView(V3d_TypeOfOrientation_Zup_Front, "_front");
    captureView(V3d_TypeOfOrientation_Zup_Top, "_top");
    captureView(V3d_TypeOfOrientation_Zup_Left, "_left");

    // Cleanup temporary lights
    view->SetLightOff(); // Disable all (including temps)
#if SetupLight
    // Remove from viewer to avoid accumulating lights
    if (!m_occView->Viewer().IsNull()) {
        m_occView->Viewer()->DelLight(ambLight);
        m_occView->Viewer()->DelLight(headLight);
    }
#endif

    // Restore original lights (assuming default behavior is all on)
    view->SetLightOn();

    // Restore camera
    view->SetCamera(savedCamera);
    view->Redraw(); // Draw the restored view to the widget
    m_occView->doneCurrent();

    QMessageBox::information(this, tr("Export"),
                             tr("Three view drawings saved to: %1").arg(QDir::currentPath()));
}
